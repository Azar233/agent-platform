...
- Android IP Webcam 的 MJPEG 实时画面预览。

当前付款页面只展示已确认的商品和金额，付款二维码、模拟支付状态同步以及真实支付均未实现。IP Webcam 也仅用于画面预览，尚未将摄像头帧发送给 YOLO。
...
- 前端 `/checkout` 可以显示手机实时画面。
- 当前没有实现抓帧检测，商品识别请使用“图片上传”。
...
flowchart LR
    ...
    Phone["IP Webcam"] --> Camera["Camera Proxy"]
    Camera --> Browser
    API --> Auth["认证与会话"]
...
6. 价格完整时可以进入 `/checkout/payment` 查看确认后的清单和金额。

付款页面当前不会生成二维码，也不会发起真实或模拟支付。
...
3. 确认电脑能访问手机显示的局域网地址。
4. 修改 `frontend/src/views/CustomerCheckoutPage.vue` 中的 `IP_WEBCAM_URL`。
5. 打开 `/checkout` 的 Webcam 模式查看画面。

后端 `/api/camera/ip-webcam/stream` 只代理私有局域网地址。当前画面不会自动抓取或发送给 YOLO。
