...
- Supervisor 通过规则和 DashScope/Chroma 模糊路由，将请求交给 Detection、Dataset、Training、Catalog 或 Knowledge Agent，并通过 SSE 流式返回结果。
- Dataset Agent 的添加样品操作使用页面交接：Agent 预检和预填，用户在数据集页面选择本地文件夹并逐图人工绘框。
...
多 Agent 模糊路由、知识库、故障案例和长期记忆使用阿里云百炼 `text-embedding-v4` 与本地 Chroma：
...
如果使用 Windows `setx DASHSCOPE_API_KEY "..."`，必须重新打开终端后再启动后端。首次构建知识库和初始化向量路由：
...
Chroma 数据保存在项目根目录 `.runtime/chroma`，不会提交到 Git。训练可以在其他机器或集群执行；管理端的 Training Agent 通过数据库任务、日志和指标进行监控，不要求当前后端机器具备 CUDA 训练环境。
