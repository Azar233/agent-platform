...
VisionPay Agent Platform 是一个面向零售自助结算场景的商品视觉识别平台。系统以 YOLOv11 完成图片、视频和实时画面的商品检测，以 FastAPI 管理数据集、训练任务、模型版本、商品与价格，以 Vue 3 提供检测、数据集版本、模型训练、自助结算和运营管理界面。
...
- 单张图片、多张图片和 ZIP 批量检测。
- MP4、AVI、MOV、MKV 视频异步检测，展示任务进度和关键帧。
- Android IP Webcam MJPEG 实时检测、跨帧稳定和实时计价。
- 返回类别、置信度、边界框、标注图、推理耗时和商品价格汇总。
- DeepSeek Agent 根据自然语言调用检测工具，并通过 SSE 流式返回结果。
...
- 按 train、val、test 三个文件夹批量添加商品图片。
- 基于背景颜色和前景连通区域生成候选检测框，并提供图形化审核与调整。
- 搜索并删除商品的全部相关图片和标注，自动重排后续 `class_index`。
- 校验、冻结、设为当前、归档和删除草稿。
- 派生版本、添加商品、删除商品和删除草稿提供实时进度。
...
flowchart LR
    ...
    API --> Agent["DeepSeek Detection Agent"]
...
| 智能体 | LangChain、LangGraph、DeepSeek OpenAI-compatible API |
...
1. 填写带 `*` 的商品名称、类别名称和价格。
2. 商品条码可选；有条码时建议填写，以便跨版本复用稳定 `product_key`。
3. 分别选择 train、val、test 文件夹。train 至少一张图片；本次新增商品的 val/test 文件夹可为空，系统不会自动拆分图片。
4. 点击“下一步”，系统暂存所有图片并自动生成候选检测框。
5. 在图形化界面检查每张图，可移动、缩放、新增或删除检测框。
6. 确认后系统批量写入图片和 YOLO `.txt`，更新 `data.yaml`、`manifest.json`、内容指纹、商品索引和价格。

自动框使用图像边缘颜色估计背景，再提取前景连通区域；复杂背景、遮挡、多商品或商品贴边时会标记需要审核。它是候选框生成器，不应替代人工确认。

注意：单次新增允许 val/test 为空，但当前冻结校验要求整个数据集的 train、val、test 都至少有图片和标注。基线已有非空验证集和测试集时不受影响。
...
2. 确认目标商品。
3. 系统删除该商品对应的全部图片和标注。若一张图同时标注了目标商品和其他商品，当前实现会删除整张图片及其标签文件。
...
1. 注册或登录。
2. 选择单图、多图、ZIP、视频或 IP Webcam 模式。
3. 设置场景、置信度和 IoU。
4. 上传文件或连接手机摄像头。
5. 查看标注图、检测框、类别统计、价格明细和总价。

上传视频会创建异步任务；前端轮询 `/api/detection/video/status/{task_id}` 展示进度。默认每 5 帧采样一次，最多保存 50 个关键帧，可通过环境变量调整。

### 9.2 智能对话 `/chat`

配置 DeepSeek 后，可上传图片或 ZIP 并用自然语言描述任务。Agent 会选择对应检测工具，通过 SSE 返回结果，并保存会话和附件记录。
...
自动框依赖边缘背景估计，复杂背景下只作为候选。使用图形化编辑器调整后再提交；如果没有可靠前景，系统会要求审核而不是自动使用整张图片。

### 新增商品的 val/test 可以为空，为什么无法冻结

为空指本次新增商品不强制提供 val/test 样本；冻结规则仍要求整个数据集的 train、val、test 分区都有图片和标注。请确保基线或草稿整体满足校验要求。
