...
每个后台账号可以在 `/settings` 的“自定义指令”中设置统一的 Agent 响应偏好，例如输出语言、语气、角色化口吻、自称方式、详细程度、结论顺序和 Markdown 表格格式。指令按 `user_id` 存入 PostgreSQL，不进入 RAG、长期记忆或聊天历史；保存后对该账号新发送的消息立即生效，并注入 Detection、Dataset、Training、Catalog 和 Knowledge Agent。自定义指令可以覆盖系统默认的专业、克制和中文等表达风格；emoji、颜文字或装饰图标只有在用户明确要求时才允许。指令只能改变回答方式，不能改变 Agent 的真实身份与职责，也不能改变 Supervisor 路由、工具参数、权限、事实来源、影响预览、确认令牌、幂等保护和操作审计。
...
业务知识和故障案例不再直接返回原始 Top K。检索前会结合 `ChatSession.context_state` 中的当前工作流和实体重写问题，例如把“这个版本怎么冻结”重写为“Dataset 数据集版本冻结条件、影响范围和操作流程”，同时保留用户原话和已知版本信息。明确的新意图优先于旧工作流，概念定义问题不会被陈旧任务状态污染。

检索时先按 `RAG_CANDIDATE_MULTIPLIER` 过召回候选，再依次执行最低相似度过滤、同文档相邻 Chunk 去重、跨文档近重复内容去重和单文档结果数量限制，最后返回 Top K。返回结果包含实际使用的 `rewritten_query`、领域、purpose、是否使用上下文以及每条结果的相似度和最终排名，便于审计检索效果。

长期记忆只允许 `preference`、`stable_fact`、`output_format` 和 `workflow` 四类稳定信息。写入时会拦截密码、Token、密钥、实时价格和临时任务状态；完全相同的内容直接去重，高相似的新偏好覆盖旧记录并增加 revision。召回同时执行用户隔离、类别过滤和最低相似度过滤，后台 API 支持分页查看、修改和删除。
...
`/chat` 没有把整段聊天记录无限追加到 Prompt，而是采用“完整事件留存、结构化任务状态、Token 感知对话窗口、按需检索增强、当轮工具上下文”的混合架构。这样既保留审计和连续操作能力，也避免长会话不断挤占上下文或让旧任务污染新意图。

#### 2.4.1 上下文分层

| 层级 | 数据来源 | 主要作用 | 生命周期与模型可见性 |
| --- | --- | --- | --- |
| 完整会话记录 | PostgreSQL `ChatMessage` | 保存用户消息、Agent 回复、路由结果、表单、附件引用、工具调用和工具结果，用于历史查询与审计 | 持久化保存；不会整段无上限发送给模型 |
| 结构化任务状态 | PostgreSQL `ChatSession.context_state` | 保存当前 Agent、未完成工作流、业务实体、已知/缺失参数和待办操作 | 会话级持久化；每轮作为系统上下文注入，并参与路由和检索问题改写 |
| Token 感知对话窗口 | `context_summary` + 近期 `ChatMessage` | 保留近期原文，把较早对话压缩为可增量更新的摘要 | 每轮动态组装，只选取预算内内容 |
| 检索增强上下文 | Chroma 中的知识文档、故障案例和经营者长期记忆 | 补充操作规范、历史故障处理方案和用户明确保存的稳定偏好 | 按问题相关性检索，不等同于聊天历史；长期记忆按 `user_id` 隔离 |
| 当轮工具上下文 | LangChain Agent scratchpad | 保存本轮工具调用参数与结果，支持 Agent 在一次执行中连续推理 | 仅当前请求有效；重要结果会脱敏、限长后写回会话记录和结构化状态 |

结构化状态 `ConversationState` 的主要字段如下：

| 字段 | 含义 |
| --- | --- |
| `active_agent` | 最近处理当前任务的领域 Agent |
| `active_workflow` | 当前工作流的领域、操作目的和状态，例如等待输入、等待页面交接或等待确认 |
| `entities` | 已确认的数据集版本、商品、模型、训练任务、场景等业务实体 ID 或名称 |
| `known_values` / `missing_fields` | 已收集参数与仍需通过结构化问询表单补充的参数 |
| `pending` | 待提交表单、页面交接或待确认操作的引用和状态 |
| `last_tool` | 最近一次领域工具的脱敏输入与结果摘要 |
| `updated_at` | 状态最近更新时间 |

这些字段主要由已校验的表单提交、领域工具返回、页面交接和确认操作等权威事件更新，不依赖模型从自然语言回复中猜测数据集 ID、任务 ID 或操作是否已经完成。

#### 2.4.2 每轮请求如何工作

```mermaid
flowchart LR
    A["用户消息、表单或附件"] --> B["从数据库校准未完成任务"]
    B --> C["按 Token 预算组装历史窗口"]
    B --> D["结合当前输入和任务状态路由"]
    B --> E["结合任务状态改写检索问题"]
    E --> F["召回相关长期记忆"]
    C --> G["领域 Agent 执行"]
    D --> G
    F --> G
    G --> H["调用领域工具或 RAG 检索工具"]
    H --> I["保存消息、用量与权威结果"]
    I --> J["更新 ConversationState"]
```

一次管理端 Agent 调用的模型输入按以下顺序形成：

1. 当前领域 Agent 的系统提示词，以及与本次问题相关的经营者长期记忆。
2. 较早对话的增量摘要和系统维护的结构化任务状态。
3. 预算内的最近消息原文，包括必要的表单提交、附件引用，以及经过脱敏和限长的工具事实。
...
当前消息始终是判断新意图的主要信号。明确的检测、数据集、训练或价目表操作可以覆盖旧会话状态；只有“继续”“这个版本”“用刚才那个任务”等依赖指代、且确实存在未完成工作流时，才使用 `active_workflow` 和 `entities` 延续原 Agent。

默认混合路由会为每条消息执行 Embedding 语义匹配，同时保留可审计的明确业务规则。路由优先级概括为：明确意图和附件规则、未完成工作流、Embedding 结果、关键词和上一轮 Agent，最终无法判断时交给 Knowledge Agent 澄清。`embedding_only` 仅用于纯向量路由测试，不建议作为生产默认值。

Embedding 主要理解当前输入的语义；结构化状态负责补充跨轮任务事实。两者分工可以在延续 Dataset 加样等流程的同时，避免旧的 Dataset 或 Training 状态错误接管一个新的知识问题。

#### 2.4.4 Token 预算与摘要策略

- 历史上下文默认预算为 6000 Token，优先保留最近 12 条消息原文。
- 超出近期窗口的消息按增量方式并入摘要，摘要最多保留 1200 Token；不会在每一轮重新总结整段历史。
- 单项工具结果进入历史上下文前最多保留 1600 个字符，并执行字段脱敏和集合限长。
- 上下文按 Token 预算裁剪，不使用固定“只看最近 20 条”之类的消息数量硬截断。
- `context_tokens` 是后端对本轮已组装历史上下文的本地估算，用于观察预算；`model_usage` 和最终记录的 `tokens_used` 来自模型返回的真实用量元数据。兼容模型不返回用量时，真实用量字段保持为空，不用估算值代替账单数据。

#### 2.4.5 RAG、长期记忆与实时状态的边界

RAG、长期记忆和业务数据库解决的是三类不同问题：

- 知识库和故障案例保存可复用的操作说明与已确认经验。检索前会结合当前任务状态改写含有“这个”“刚才”等指代的问题，检索后执行相似度阈值过滤、相邻片段去重和单来源数量限制。
- 长期记忆只保存经营者明确要求记住的稳定偏好、稳定事实、输出格式和工作习惯，并提供用户隔离、相似内容去重/覆盖以及增删改查能力。
- 实时价格、数据集状态、训练进度、模型状态和待确认操作必须通过对应领域工具查询数据库，不能从 RAG、长期记忆或早期摘要中推断。

密码、API Key、Token、确认令牌、服务器文件路径等敏感信息不会进入结构化任务状态；密码、密钥、Token、实时价格和临时任务状态也禁止写入长期记忆。页面交接、确认、取消或失败发生在其他 API 时，下一轮会先从数据库权威记录重新校准状态，避免继续使用已经失效的工作流。
...
RAG_DEDUP_SIMILARITY=0.88
RAG_ADJACENT_DEDUP_SIMILARITY=0.50
...
LONG_TERM_MEMORY_MIN_SIMILARITY=0.55
LONG_TERM_MEMORY_DEDUPE_SIMILARITY=0.90
AGENT_CONTEXT_HISTORY_TOKENS=6000
AGENT_CONTEXT_SUMMARY_TOKENS=1200
AGENT_CONTEXT_RECENT_MESSAGES=12
AGENT_CONTEXT_TOOL_RESULT_CHARS=1600
...
| `POST` | `/api/knowledge/search` | 检索业务知识，可指定 `session_uuid`、`domain`、`top_k` 和最低相似度，并返回重写问题 |
| `POST` | `/api/knowledge/fault-cases` | 写入人工确认的动态故障案例 |
| `POST` | `/api/knowledge/memory/search` | 按当前登录用户、类别和最低相似度检索长期记忆 |
| `POST` | `/api/knowledge/memory` | 保存长期记忆；相同内容去重，相似偏好覆盖 |
| `GET` | `/api/knowledge/memory` | 分页查看当前用户的长期记忆，可按类别过滤 |
| `GET` | `/api/knowledge/memory/{id}` | 查看当前用户的一条长期记忆 |
| `PUT` | `/api/knowledge/memory/{id}` | 修改长期记忆并合并高相似重复项 |
| `DELETE` | `/api/knowledge/memory/{id}` | 删除当前用户的一条长期记忆 |

这些接口均要求登录认证并按 `user_id` 隔离。对话内 Knowledge Agent 仍只在用户明确要求记住时调用 `remember_management_preference`；管理 API 用于经营者主动查看、维护和删除自己的长期记忆。
...
- “识别记录”展示当前账号的单图、批量、ZIP、视频和实时摄像头检测任务，并可查看检测框与类别明细。
- “Agent 调用”展示当前账号最近使用的 Agent、用户请求、执行动作、工具链、路由依据、附件、耗时和最终回复。
- “模型历史”展示活动及已归档模型的创建来源、创建时间、数据集与训练任务，并以时间线记录设为默认、取消默认、评估、导出、下载和归档等操作。

Agent 调用历史严格按登录用户隔离；模型版本及生命周期与模型管理页保持一致，对已登录的后台管理人员可见。旧模型会根据 `ModelVersion.created_at` 显示可靠的创建事件；本版本启用后的模型操作会持续写入模型级审计日志。
...
| `AGENT_CONTEXT_HISTORY_TOKENS` | `6000` | 每轮注入模型的会话历史 Token 预算 |
| `AGENT_CONTEXT_SUMMARY_TOKENS` | `1200` | 较早对话增量摘要的最大 Token 数 |
| `AGENT_CONTEXT_RECENT_MESSAGES` | `12` | 优先保留原文的最近消息数量 |
| `AGENT_CONTEXT_TOOL_RESULT_CHARS` | `1600` | 上下文中单项工具结果的最大字符数 |
| `RAG_MIN_SIMILARITY` | `0.45` | 业务知识和故障案例最低召回相似度 |
| `RAG_DEDUP_SIMILARITY` | `0.88` | RAG 结果内容去重阈值 |
| `RAG_ADJACENT_DEDUP_SIMILARITY` | `0.50` | 同一文档相邻切片判定为重复的相似度阈值 |
| `RAG_MAX_CHUNKS_PER_SOURCE` | `2` | 单份源文档最多返回的非相邻片段数 |
| `RAG_CANDIDATE_MULTIPLIER` | `4` | 去重前候选过召回倍数 |
| `LONG_TERM_MEMORY_MIN_SIMILARITY` | `0.55` | 长期记忆最低召回相似度 |
| `LONG_TERM_MEMORY_DEDUPE_SIMILARITY` | `0.90` | 相似记忆覆盖和合并阈值 |
...
| `ACCESS_TOKEN_EXPIRE_MINUTES` | `10080` | 登录令牌有效期（分钟，默认 7 天） |
...
| `/api/history` | 检测任务、Agent 调用和模型生命周期统一历史 |
