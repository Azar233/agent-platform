...
缺参时必须调用固定表单：查询价目表使用 catalog.list_prices，改价使用 catalog.update_price，清除价格使用 catalog.clear_price。用户原话中的商品名、目标价格等必须放入 known_values 预填；可用数据集版本通过 option_overrides.dataset_version_id 传入，不能改变模板字段。
""" + COMMON_RULES
