...
.history-page { min-height: 100%; padding: 32px; display: flex; flex-direction: column; gap: 18px; color: $text-primary; background: $bg-color; }
.page-header { padding: 8px 0 14px; display: flex; align-items: center; justify-content: space-between; gap: 20px; }.page-header h1 { margin: 10px 0 0; color: $text-primary; font-family: 'Space Grotesk', 'DM Sans', sans-serif; font-size: 40px; line-height: 1.08; letter-spacing: 0; }.page-header p { max-width: 560px; margin: 12px 0 0; color: $text-secondary; font-size: 16px; line-height: 1.6; }
...
:global(html.dark .summary-grid article) { background: rgba(28,28,30,.88); border-color: rgba(255,255,255,.1); }:global(html.dark .history-tabs .el-tabs__header) { background: rgba(22,22,24,.9); border-color: rgba(255,255,255,.1); }
@media (max-width: 1050px) { .summary-grid { grid-template-columns: repeat(2,1fr); } }
@media (max-width: 760px) { .history-page { padding: 20px; }.page-header { align-items: flex-start; flex-direction: column; }.page-header h1 { font-size: 30px; } }
@media (max-width: 680px) { .summary-grid { grid-template-columns: 1fr; }.history-tabs :deep(.el-tabs__item) { padding: 0 2px; }.tab-label { padding: 0 8px; }.tab-label small { display: none; } }
</style>
